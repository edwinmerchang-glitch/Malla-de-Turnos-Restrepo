...
# En la sección "Matriz turnos" (op == "Matriz turnos"), reemplaza el bloque con tab1

with tab1:
    st.markdown("### Vista de matriz de turnos")
    
    # Tu código existente para generar data...
    data = []
    for emp in empleados:
        fila = {
            "Empleado": emp.nombre,
    ...
    if data:
        ...
        # Agregar controles para personalizar la vista
        col1, col2, col3 = st.columns(3)
        with col1:
            alto_tabla = st.slider("Altura de la tabla", min_value=300, max_value=800, value=500, step=50)
        with col2:
            ancho_col_fija = st.slider("Ancho columnas fijas", min_value=150, max_value=300, value=200, step=10)
        with col3:
            ancho_col_dia = st.slider("Ancho columnas días", min_value=50, max_value=120, value=70, step=5)
        
        # Convertir DataFrame a HTML con estilo personalizado
        ...
        /* Colores para diferentes tipos de turno */
        .matriz-turnos td {{
        ...
        .matriz-turnos td:hover::after {{
            content: attr(data-turno);
            position: absolute;
            bottom: 100%;
            left: 50%;
                    ...
                    if st.button("♻️ Restaurar backup", use_container_width=True, type="primary", disabled=not confirmar):
                        try:
                            ...
                            with open("data.db", "wb") as f:
                                f.write(archivo_subido.getbuffer())import streamlit as st
import pandas as pd
from datetime import date, timedelta, datetime
from database import Session, Empleado, Turno, Asignacion
from scheduler import generar_malla_inteligente
